...
const Nfts = () => {
  ...
  const { view: view, display: initialDisplay, sortBy: initialSort, asc, search: initialSearch } = router.query
  ...
  const [sortBy, setSortBy] = useState(initialSort ? initialSort : 'created_at')
  const [sortAsc, setSortAsc] = useState(asc ? asc : false)
  ...
  if (!nfts || loading) return <div className='flex items-center justify-center mt-32'><PulseLoader color={'var(--color-cta)'} size={20} /></div>
        ...
        <div className='mb-10 flex justify-between w-full border-b-2 border-detail dark:border-detail-dark'>
          ...
          <div className='flex items-center justify-center gap-6'>
            ...
            <button className='uppercase hover:text-cta' onClick={() => sortByDatePrice('created_at')}>
              <span className={`font-serif ${sortBy === 'created_at' ? `text-cta` : `text-detail dark:text-detail-dark hover:text-cta dark:hover:text-cta`}`}>Date</span>
            </button>
            <button className='uppercase hover:text-cta' onClick={() => sortByDatePrice('price')}>
              <span className={`font-serif ${sortBy === 'price' ? `text-cta` : `text-detail dark:text-detail-dark hover:text-cta dark:hover:text-cta`}`}>Price</span>
            </button>
